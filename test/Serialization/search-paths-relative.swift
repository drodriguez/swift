...
// CHECK: <XCC abbrevid={{[0-9]+}}/> blob data = 'PATH((.+/secret))'
// CHECK: <XCC abbrevid={{[0-9]+}}/> blob data = '-DDUMMY'
// CHECK: </OPTIONS_BLOCK>

// CHECK-LABEL: <INPUT_BLOCK
// CHECK: <SEARCH_PATH abbrevid={{[0-9]+}} op0=1 op1=0/> blob data = 'PATH((.+/secret/../Frameworks))'
// CHECK: <SEARCH_PATH abbrevid={{[0-9]+}} op0=0 op1=0/> blob data = 'PATH((.+/secret/.))'
// CHECK: </INPUT_BLOCK>
