 ...
 */
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=address -target x86_64-apple-macosx10.9 %s | %FileCheck --enable-yaml-compatibility -check-prefix=ASAN -check-prefix=ASAN_OSX %s
...
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=address -target x86_64-apple-ios7.1 %s | %FileCheck --enable-yaml-compatibility -check-prefix=ASAN -check-prefix=ASAN_IOSSIM %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=address -target arm64-apple-ios7.1 %s  | %FileCheck --enable-yaml-compatibility -check-prefix=ASAN -check-prefix=ASAN_IOS %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=address -target x86_64-apple-tvos9.0 %s | %FileCheck --enable-yaml-compatibility -check-prefix=ASAN -check-prefix=ASAN_tvOS_SIM %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=address -target arm64-apple-tvos9.0 %s  | %FileCheck --enable-yaml-compatibility -check-prefix=ASAN -check-prefix=ASAN_tvOS %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=address -target i386-apple-watchos2.0 %s   | %FileCheck --enable-yaml-compatibility -check-prefix=ASAN -check-prefix=ASAN_watchOS_SIM %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=address -target armv7k-apple-watchos2.0 %s | %FileCheck --enable-yaml-compatibility -check-prefix=ASAN -check-prefix=ASAN_watchOS %s
 ...
 */
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=thread -target x86_64-apple-macosx10.9 %s | %FileCheck --enable-yaml-compatibility -check-prefix=TSAN -check-prefix=TSAN_OSX %s
 ...
 */
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target x86_64-apple-macosx10.9 %s | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN -check-prefix=UBSAN_OSX %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target x86_64-apple-ios7.1 %s | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN -check-prefix=UBSAN_IOSSIM %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target arm64-apple-ios7.1 %s  | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN -check-prefix=UBSAN_IOS %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target x86_64-apple-tvos9.0 %s | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN -check-prefix=UBSAN_tvOS_SIM %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target arm64-apple-tvos9.0 %s  | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN -check-prefix=UBSAN_tvOS %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target i386-apple-watchos2.0 %s   | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN -check-prefix=UBSAN_watchOS_SIM %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target armv7k-apple-watchos2.0 %s | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN -check-prefix=UBSAN_watchOS %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target x86_64-unknown-linux-gnu %s 2>&1 | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN_LINUX %s
// RUN: %swiftc_driver -resource-dir %S/Inputs/fake-resource-dir/lib/swift/ -driver-print-jobs -sanitize=undefined -target x86_64-unknown-windows-msvc %s 2>&1 | %FileCheck --enable-yaml-compatibility -check-prefix=UBSAN_WINDOWS %s
...
// ASAN_OSX: PATH((lib/swift/clang/lib/darwin/libclang_rt.asan_osx_dynamic.dylib))
// ASAN_IOSSIM: PATH((lib/swift/clang/lib/darwin/libclang_rt.asan_iossim_dynamic.dylib))
// ASAN_IOS: PATH((lib/swift/clang/lib/darwin/libclang_rt.asan_ios_dynamic.dylib))
// ASAN_tvOS_SIM: PATH((lib/swift/clang/lib/darwin/libclang_rt.asan_tvossim_dynamic.dylib))
// ASAN_tvOS: PATH((lib/swift/clang/lib/darwin/libclang_rt.asan_tvos_dynamic.dylib))
// ASAN_watchOS_SIM: PATH((lib/swift/clang/lib/darwin/libclang_rt.asan_watchossim_dynamic.dylib))
// ASAN_watchOS: PATH((lib/swift/clang/lib/darwin/libclang_rt.asan_watchos_dynamic.dylib))
// ASAN_LINUX: -fsanitize=address
// ASAN_WINDOWS: PATH((lib/swift/clang/lib/windows/clang_rt.asan-x86_64.lib))
...
// TSAN_OSX: PATH((lib/swift/clang/lib/darwin/libclang_rt.tsan_osx_dynamic.dylib))
...
// UBSAN_OSX: PATH((lib/swift/clang/lib/darwin/libclang_rt.ubsan_osx_dynamic.dylib))
// UBSAN_IOSSIM: PATH((lib/swift/clang/lib/darwin/libclang_rt.ubsan_iossim_dynamic.dylib))
// UBSAN_IOS: PATH((lib/swift/clang/lib/darwin/libclang_rt.ubsan_ios_dynamic.dylib))
// UBSAN_tvOS_SIM: PATH((lib/swift/clang/lib/darwin/libclang_rt.ubsan_tvossim_dynamic.dylib))
// UBSAN_tvOS: PATH((lib/swift/clang/lib/darwin/libclang_rt.ubsan_tvos_dynamic.dylib))
// UBSAN_watchOS_SIM: PATH((lib/swift/clang/lib/darwin/libclang_rt.ubsan_watchossim_dynamic.dylib))
// UBSAN_watchOS: PATH((lib/swift/clang/lib/darwin/libclang_rt.ubsan_watchos_dynamic.dylib))
// UBSAN_LINUX: -fsanitize=undefined
// UBSAN_WINDOWS: PATH((lib/swift/clang/lib/windows/clang_rt.ubsan-x86_64.lib))
