...
// RUN: %target-swift-frontend -primary-file %/s -S -g -o - | %FileCheck --enable-yaml-compatibility %s
...
// CHECK: .loc	[[MAIN]] 8
// CHECK: .loc	[[ABC]] 142
// CHECK: .file	[[DEF:.*]] "PATH((//absolute/path/def.swift))"
// CHECK: .loc	[[DEF]] 142
// CHECK: .asciz "{{.*}}test/DebugInfo"

// RUN: %empty-directory(%t)
// RUN: sed -e "s|LINE_DIRECTIVE_DIR|%/S|g" %S/Inputs/vfsoverlay.yaml > %t/overlay.yaml
// RUN: %target-swift-frontend -vfsoverlay %t/overlay.yaml -primary-file %/S/vfs-relocated-line-directive.swift -S -g -o - | %FileCheck --enable-yaml-compatibility -check-prefix=VFS %s
...
// VFS: .loc  [[MAIN]] 8
// VFS: .loc  [[ABC]] 142
// VFS: .file  [[DEF:.*]] "PATH((//absolute/path/def.swift))"
// VFS: .loc  [[DEF]] 142
